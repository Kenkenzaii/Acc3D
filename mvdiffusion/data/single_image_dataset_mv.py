...
class SingleImageDataset(Dataset):
    def __init__(self,
        ...
        self.view_list = ["rgb_006_front.webp", "rgb_006_front_right.webp", "rgb_006_left.webp", "rgb_006_right.webp", "rgb_006_front_left.webp", "rgb_006_back.webp"]
        self.file_list = None
        ...
        if single_image is not None:
            ...
            for file in tqdm(self.file_list):
                ...
                image, alpha = self.load_image(os.path.join(self.root_dir, file, random_view), bg_color, return_type='pt')
...
class SingleImageDatasetMV(Dataset):
    def __init__(self,
        root_dir: str,
        num_views: int,
        img_wh: Tuple[int, int],
        ...
        self.image_fix_key = ["normals_front_masked", 
                              ...
                              "color_front_masked", 
                              "color_front_right_masked", 
                              "color_right_masked",   
                              "color_back_masked",
                              "color_left_masked",
                              "color_front_left_masked"]
        ...
        for folder in self.folder_list:
            ...
            for key in self.image_fix_key:
                image, alpha = self.load_image(os.path.join(self.root_dir, folder, key + ".png"), bg_color, return_type='pt')
                images_in_folder.append(image)
                alphas_in_folder.append(alpha)
            self.all_images.append(images_in_folder)
            self.all_alphas.append(alphas_in_folder)
    ...
    def __getitem__(self, index):
        # image = self.all_images[index%len(self.all_images)]
        # alpha = self.all_alphas[index%len(self.all_images)]
        imgs_mv = self.all_images[index]
        alphas_mv = self.all_alphas[index]

        # if self.file_list is not None:
        #     filename = self.file_list[index%len(self.all_images)].replace(".png", "")
        # else:
        #     filename = 'null'
        img_tensors_in = [img.permute(2, 0, 1) for img in imgs_mv]
        alpha_tensors_in = [img.permute(2, 0, 1) for img in alphas_mv]
